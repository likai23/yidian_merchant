...
public class CustomerPayAddcountController extends AbstractController<CustomerPayAddcountService, CustomerPayAddcount> {
	...
	public JsonResult<IPage<CustomerDataAndPayAddcount>> selectPayAddcountPage(
		...
		// 分页数据
		IPage<CustomerDataAndPayAddcount> pageData = baseService.selectCustomerPayAddPageIndex(page,
	...
	@ApiOperation(value = "添加充值记录", notes = "作者：戴艺辉")
	public JsonResult<Object> saveCustomerPayAddcount(@RequestBody CustomerPayAddcountDto entity) {
		...
		if (null != entity) {
			...
			customerPayAddcount.setRechargeAmount(
					new BigDecimal(entity.getRechargeAmount()).multiply(new BigDecimal("10000")).longValue());
